...
/**
 * BSF Analytics integration: opt-in, deactivation survey, and stats payload.
 *
 * Named `Bsf_Analytics` rather than `Analytics` so it cannot be confused with
 * `Power_Coupons\Includes\Power_Coupons_Analytics`, which owns the Power Coupons
 * stats payload itself. The two used to share a class name, differing only by
 * namespace, so a missing `use` statement resolved silently to the wrong one.
 *
 * @since 1.1.0
 */
class Power_Coupons_Bsf_Analytics {
	...
	protected function __construct() {
		...
		if ( ! class_exists( 'BSF_Analytics_Loader' ) ) {
			require_once POWER_COUPONS_DIR . 'lib/bsf-analytics/class-bsf-analytics-loader.php';
		}
		...
		$bsf_analytics->set_entity(
			[
				'power-coupons' => [
					...
					'path'                => POWER_COUPONS_DIR . 'lib/bsf-analytics',
	...
	private function get_coupons_with_rules_count() {
		// Single implementation lives in the always-loaded Includes class; this
		// class is only required under is_admin(), so the dependency runs that
		// way round and cannot invert.
		return \Power_Coupons\Includes\Power_Coupons_Analytics::count_rule_enabled_coupons();
	}
